  ...
  },
{
    ...
    tests: [
      ...
      { name: "N below 1 counts nothing", body: "assert.equal(countStrictlyIncreasing(0), 0);\nassert.equal(countStrictlyIncreasing(-5), 0);" },
      { name: "counts by length are binomial coefficients", body: "assert.equal(countStrictlyIncreasing(999), 129, \"9 + 36 + 84\");\nassert.equal(countStrictlyIncreasing(9999), 255);\nassert.equal(countStrictlyIncreasing(99999), 381);\nassert.equal(countStrictlyIncreasing(999999), 465);" },
      { name: "either side of the largest possible value", body: "assert.equal(countStrictlyIncreasing(123456789), 511);\nassert.equal(countStrictlyIncreasing(123456788), 510);" },
      { name: "the count grows by at most one per step", body: "let prev = countStrictlyIncreasing(0);\nfor (let n = 1; n <= 3000; n++) {\n  const c = countStrictlyIncreasing(n);\n  assert.ok(c === prev || c === prev + 1, \"jump at \" + n);\n  prev = c;\n}" },
      { name: "larger values agree with brute force", body: "const naive = (limit) => { let c = 0; for (let v = 1; v <= limit; v++) { const t = String(v); let ok = true; for (let i = 1; i < t.length; i++) if (t.charCodeAt(i) <= t.charCodeAt(i - 1)) ok = false; if (ok) c++; } return c; };\nfor (const limit of [3210, 4567, 9876, 12345, 54321]) assert.equal(countStrictlyIncreasing(limit), naive(limit), \"N = \" + limit);" },
    ],
  },
{
  ...
  },
{
    ...
    tests: [
      ...
      { name: "two cuts cost the same whichever is made first", body: "assert.equal(minCost(10, [3, 7]), 17);" },
      { name: "the input array is not reordered", body: "const cuts = [5, 1, 4];\nminCost(7, cuts);\nassert.deepEqual(cuts, [5, 1, 4]);" },
      { name: "reflecting the stick gives the same cost", body: "const n = 20;\nconst cuts = [3, 8, 11, 15];\nassert.equal(minCost(n, cuts), minCost(n, cuts.map((c) => n - c)));" },
      { name: "agrees with a brute-force search on random small sticks", body: "const brute = (n, cuts) => { const memo = new Map(); const f = (lo, hi) => { const key = lo + \",\" + hi; if (memo.has(key)) return memo.get(key); let best = 0, any = false; for (const c of cuts) if (c > lo && c < hi) { const v = hi - lo + f(lo, c) + f(c, hi); if (!any || v < best) { best = v; any = true; } } memo.set(key, best); return best; }; return f(0, n); };\nlet x = 11;\nfor (let t = 0; t < 30; t++) {\n  x = (x * 1103515245 + 12345) & 0x7fffffff;\n  const n = 10 + (x % 20);\n  const cuts = [];\n  for (let c = 1; c < n; c++) { x = (x * 1103515245 + 12345) & 0x7fffffff; if (x % 3 === 0) cuts.push(c); }\n  assert.equal(minCost(n, cuts), brute(n, cuts), \"n=\" + n + \" cuts=\" + cuts.join(\",\"));\n}" },
      { name: "a hundred cuts finish quickly", body: "const cuts = Array.from({ length: 100 }, (_, i) => (i + 1) * 10);\nconst r = minCost(1010, cuts);\nassert.ok(r > 1010 && Number.isFinite(r));" },
    ],
  },
{
  ...
  },
{
    ...
    tests: [
      ...
      { name: "a negative size does no comparisons", body: "assert.equal(countComparisons(-4), 0);" },
      { name: "the count grows by n at each step", body: "for (let n = 1; n <= 300; n++) assert.equal(countComparisons(n + 1) - countComparisons(n), n, \"at \" + n);" },
      { name: "three and four", body: "assert.equal(countComparisons(3), 3);\nassert.equal(countComparisons(4), 6);" },
      { name: "the answer is always an integer", body: "for (const n of [7, 99, 1001, 40001, 3000000]) assert.ok(Number.isInteger(countComparisons(n)), String(n));" },
      { name: "the ratio to n squared tends to one half", body: "const n = 100000;\nconst ratio = countComparisons(n) / (n * n);\nassert.ok(ratio > 0.4999 && ratio < 0.5, String(ratio));" },
    ],
  },
{
    ...
    tests: [
      ...
      { name: "both sides of every boundary", body: "const at = (n) => pickApproach(n);\nassert.equal(at(19), at(20));\nassert.notEqual(at(20), at(21));\nassert.equal(at(299), at(300));\nassert.notEqual(at(300), at(301));\nassert.equal(at(4999), at(5000));\nassert.notEqual(at(5000), at(5001));\nassert.equal(at(999999), at(1000000));\nassert.notEqual(at(1000000), at(1000001));" },
      { name: "only the five allowed answers ever appear", body: "const allowed = [\"O(2^n) / bitmask\", \"O(n^3)\", \"O(n^2)\", \"O(n log n)\", \"O(n)\"];\nfor (const n of [1, 2, 5, 20, 21, 100, 300, 301, 1000, 5000, 5001, 10000, 1000000, 1000001, 10 ** 9]) assert.ok(allowed.includes(pickApproach(n)), String(n));" },
      { name: "the intended cost never gets worse as n grows", body: "const rank = { \"O(n)\": 0, \"O(n log n)\": 1, \"O(n^2)\": 2, \"O(n^3)\": 3, \"O(2^n) / bitmask\": 4 };\nlet prev = rank[pickApproach(1)];\nfor (const n of [1, 10, 20, 21, 100, 300, 301, 1000, 5000, 5001, 100000, 1000000, 1000001, 10000000]) {\n  const r = rank[pickApproach(n)];\n  assert.ok(r <= prev, \"n=\" + n + \" got a more expensive answer than a smaller n\");\n  prev = r;\n}" },
      { name: "the smallest and largest inputs", body: "assert.equal(pickApproach(0), \"O(2^n) / bitmask\");\nassert.equal(pickApproach(10 ** 12), \"O(n)\");" },
    ],
  },
{
