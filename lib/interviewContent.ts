...
export const interviewNotes: NotesFile = {
  meta: {
    ...
    lead: "Twelve rounds in the order you meet them, plus the three screening formats that decide a loop before anyone talks to you — the online assessment, the take-home and the technical phone screen — a frontend round about the browser rather than the framework, and Amazon's Leadership Principles loop, which is scored differently from every other round in here. Eleven of the questions every loop asks carry a ladder — the same answer at two years, at six, and at ten, with the words to say at each. With the scouting report on how your own resume reads from the other side of the table, and a week-before prep plan — the ₹20–28L core loop. The scripts are worked through on one example profile and one example salary; substitute your own and they still hold. Plus what changes when the employer is large (levels, RSUs, refreshers) or abroad (timezones, EOR versus contractor, and what the number looks like in dollars). Under Advanced: the ₹50L+ staff track, for the two-jump plan to get there.",
    author: "Akshat",
